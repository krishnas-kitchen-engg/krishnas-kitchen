...
|---|---|---|---|---|---|
| DD-001 | Temporary volunteer permissions conflict. Auth docs list limited inventory transfer/consume/return permissions, while the permissions matrix says temporary volunteers cannot receive, transfer, consume, return, adjust, or undo inventory and receive only `volunteer_sessions.create`. Mobile UI docs also describe temporary volunteers performing receive/transfer/return workflows. | `docs/AUTH_ARCHITECTURE.md`; `docs/PERMISSIONS_MATRIX.md`; `docs/features/inventory_mobile_ui_epic.md`; `docs/features/inventory_mobile_ui.md`; `docs/handbook/reference/OPEN_DECISIONS.md` | Security-sensitive implementation work may use the wrong permission model. | Create a canonical permission/reference page or ADR update that defines temporary volunteer capability by session type, then update affected docs. | High |
