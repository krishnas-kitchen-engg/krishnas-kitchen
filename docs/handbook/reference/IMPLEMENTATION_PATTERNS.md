...
| Delivery Management boundary | Use Delivery Management for product-facing capability status, delivery status, and readiness assessment. Do not use it to redefine product scope or engineering workflow. | [Delivery Model](../../delivery/DELIVERY_MODEL.md), [Product Horizons](../../PRODUCT_HORIZONS.md), [AI Engineering Operating Model](../governance/AI_ENGINEERING_OPERATING_MODEL.md) |
...
| Recipe domain foundation | Start recipe work with pure domain types, normalization, and validation that reuse shared `EntityId` and `ItemUnit`; add UI and persistence workflows only through later approved milestones. | `apps/web/src/domains/recipes/domain/recipeDefinition.ts`, [Product Horizons](../../PRODUCT_HORIZONS.md) |
| Recipe scaling | Scale recipe quantities from validated base servings to target servings inside the recipe domain. Preserve item references, units, and notes; do not perform unit conversion or availability checks in the scaling helper. | `apps/web/src/domains/recipes/domain/recipeDefinition.ts` |
| Recipe availability | Evaluate recipe readiness as pure domain composition between normalized recipe ingredients and projected inventory item balances. Match exact `itemId` plus `unit`, allocate balance per recipe line, and defer unit conversion, reservations, persistence, and shopping-list generation. | `apps/web/src/domains/recipes/domain/recipeAvailability.ts` |
| Recipe shopping list | Generate shopping-list items from recipe availability results. Group by exact `itemId` plus `unit`, include only groups with a shortage, and defer UI, persistence, unit conversion, vendors, approvals, and procurement behavior. | `apps/web/src/domains/recipes/domain/recipeShoppingList.ts` |
| Recipe repository contract | Define recipe data access in the recipe application layer before UI or persistence. Keep list/read contracts scoped by organization and optional temple, and defer Supabase adapters, migrations, RLS/RPC, authorization, and offline behavior. | `apps/web/src/domains/recipes/application/recipeRepository.ts`, [ADR-0006](../adrs/0006-repository-pattern.md) |
