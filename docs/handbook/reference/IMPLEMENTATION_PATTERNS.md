...
| Recipe domain foundation | Start recipe work with pure domain types, normalization, and validation that reuse shared `EntityId` and `ItemUnit`; add UI, persistence, availability, scaling, and shopping-list workflows only through later approved milestones. | `apps/web/src/domains/recipes/domain/recipeDefinition.ts`, [Product Horizons](../../PRODUCT_HORIZONS.md) |
