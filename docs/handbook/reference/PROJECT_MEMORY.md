...
- Recipe work has started as a Horizon 1 foundation. It currently covers input types, supported item units, required item references, positive finite quantities, servings/yield validation, normalization, recipe-specific validation errors, ingredient quantity scaling, exact item/unit availability evaluation, shortage-only shopping-list output, and a scoped recipe repository contract only.
...
| Recipe work can expand into planning too early | Recipe definitions naturally lead to availability, shopping lists, menu planning, procurement, and forecasting | The first recipe milestone stayed pure-domain and reused inventory item/unit concepts | Keep Horizon 1 recipe implementation slices vertical and explicit; defer higher-horizon planning and operations behavior |
| Recipe scaling can imply unit conversion | Scaling quantities and converting between units are different domain problems | The scaling milestone uses exact units and changes only ingredient quantities | Keep unit conversion out of recipe scaling until a separate approved milestone defines conversion rules |
