...
- Recipe work has started as a pure Horizon 1 domain foundation. It currently covers input types, supported item units, required item references, positive finite quantities, servings/yield validation, normalization, recipe-specific validation errors, ingredient quantity scaling, exact item/unit availability evaluation, and shortage-only shopping-list output only.
