...
|---|---|---|---|
| Architecture Health | Good with known gaps | ADRs and architecture references establish inventory ledger, auth/RLS boundary, PWA direction, offline sync boundary, security architecture, repositories, RPC boundaries, package organization, recipe definition/scaling/availability/shopping-list domain foundations, and recipe repository contract | Keep architecture updates tied to approved active-horizon milestones |
...
| Roadmap Progress | Conditional | Temporary volunteer permission drift, undo/reversal terminology drift, return semantics drift, offline architecture drift, security architecture ownership, living state refresh, security review baseline recording, EOS Session Controller freeze, recipe definition foundation, recipe scaling foundation, recipe availability foundation, recipe shopping-list foundation, and recipe repository contract foundation have been reconciled | Select the next active-horizon application or documentation micro-milestone through the Session Controller after the current human-review gate |
