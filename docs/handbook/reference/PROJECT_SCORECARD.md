...
|---|---|---|---|
| Architecture Health | Good with known gaps | ADRs and architecture references establish inventory ledger, auth/RLS boundary, PWA direction, offline sync boundary, security architecture, repositories, RPC boundaries, and package organization | Keep architecture updates tied to approved active-horizon milestones |
| Security Health | Watch | RLS, helper functions, authenticated read policies, volunteer RPCs, security architecture, and security ADRs exist; temporary volunteer browser permissions are restricted to read/session capabilities | Record latest full security review before production hardening or permission expansion |
...
| Roadmap Progress | Conditional | Temporary volunteer permission drift, undo/reversal terminology drift, return semantics drift, offline architecture drift, and security architecture ownership have been reconciled; current living milestone state is being refreshed after the security architecture commit | Complete the living state refresh, then select the next application or documentation micro-milestone through EOS |
