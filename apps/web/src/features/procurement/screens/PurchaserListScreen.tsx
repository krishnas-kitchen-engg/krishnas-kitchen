...
function groupItemsByPurchaseLocation(
  items: ReturnType<typeof usePurchaserList>["items"],
  purchaseLocations: ReturnType<typeof usePurchaserList>["purchaseLocations"]
) {
  const locationsById = new Map(purchaseLocations.map((location) => [location.id, location]));
  ...
  for (const item of items) {
    const purchaseLocation = item.purchaseLocationId
      ? locationsById.get(item.purchaseLocationId)
      : null;
    const groupKey = purchaseLocation?.id ?? "unassigned";
    ...
    groupedItems.set(groupKey, {
      description: purchaseLocation?.description ?? "No purchase source assigned yet.",
      items: [item],
      title: purchaseLocation?.name ?? "Unassigned source"
    });
  }
...
export function PurchaserListScreen() {
  ...
  const purchaseLocationGroups = groupItemsByPurchaseLocation(
    purchaserList.items,
    purchaserList.purchaseLocations
  );
  ...
  return (
    <section className="space-y-5">
      ...
      ) : purchaseLocationGroups.length > 0 ? (
        <section className="space-y-4">
          {purchaseLocationGroups.map((group) => (
            <section
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h2 className="text-base font-semibold text-stone-950">{group.title}</h2>
                  ...
                  >
                    ...
                    <label className="block text-sm font-medium text-stone-800">
                      ...
                      <textarea
                        ...
                        onChange={(event) =>
                          purchaserList.setDraftNotes(item.id, event.target.value)
                        }
                      ...
                      {receiptOcr?.result ? (
                        <div className="space-y-2 rounded-md border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-950">
                          ...
                          <p className="text-xs text-emerald-800">
                            Review these suggestions before uploading. Inventory is updated only
                            through receiving.
                          </p>
                        </div>
                      ) : null}
                    ...
                    <section className="space-y-3 rounded-md border border-brand-100 bg-brand-50 p-3">
                      ...
                      <button
                        ...
                        disabled={
                          !canReceiveItem || isReceiveSubmitting || !draft?.receiveLocationId
                        }
                        onClick={() => {
