...
export function ScanBarcodeEntry({
  error,
  format,
  isLoading,
  onFormatChange,
  ...
  onResolve,
  rawValue
}: ScanBarcodeEntryProps) {
  return (
    <section className="space-y-3 rounded-md border border-stone-200 bg-white p-4">
      <div>
        <h2 className="text-base font-semibold text-stone-950">Barcode entry</h2>
        <p className="mt-1 text-sm text-stone-600">Enter a barcode to identify an item.</p>
      </div>
      ...
      <button
        ...
        type="button"
      >
        {isLoading ? "Looking up..." : "Identify item"}
      </button>
    </section>
  );
}
